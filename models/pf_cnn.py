...
class PF_CNN(nn.Module):
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, labels=None, input_t_ids=None, input_t_mask=None,
        ...
        # x, aspect = inputs[0], inputs[1]
        # embedded = self.embedding(x)
        embedded = all_encoder_layers[-1]
        embedded = embedded.unsqueeze(1)  # 表示第一维度值为1，则不变，否则添加那个维度
        conved = [F.relu(conv(embedded)).squeeze(3) for conv in self.convs]
        pooled = [F.max_pool1d(conv, conv.shape[2]).squeeze(2) for conv in conved]  # huang的论文上是max_pooling
        # pooled = [F.avg_pool1d(conv, conv.shape[2]).squeeze(2) for conv in conved]

        # t_embedded = self.embedding_t(aspect)
        # t_embedded = self.embedding(aspect)
        ...
        cat = self.dropout(torch.cat([torch.cat(pooled, dim=1), torch.cat(p_pooled, dim=0)], 1))
